...
class Game(): #Track scores, settings, players etc
    ...
    def play(self):
        ...
        if event == 'q':
            return -1 #Tell manage_game() to quit
        elif not game_over == '':
...
class AI(Snake):
    ...
    def decide_next_direction(self, difference):
        ...
        difference.x = abs(difference.x) #convert all numbers to positive
        difference.y = abs(difference.y) #convert all numbers to positive
    ...
    def generate_path(self, food, other_snake):
        ...
        while not self.check_path(current_position.x, current_position.y, food.block.rect) and not force_end:
            ...
            if not chosen_direction: #could not find a suitable path so use existing path
    ...
    def generate_path2(self,food):

        #need to check new location against entire path
        ...
        in_bounds = True #flag to track that the path hasn't gone out of bounds
        while not self.check_path(current_position.x, current_position.y, food.block.rect) and in_bounds:
            print('Path Move No. ' + str(counter))
            distance = self.find_distance_to_food(food,current_position) # get distance
            old_direction = current_direction #store old direction
            current_direction = self.decide_next_direction(distance) #find next direction
            modifier = Position(self.moves[current_direction].x, self.moves[current_direction].y)

            if self.check_direction(current_direction, old_direction) and self.check_move(current_position.x+modifier.x,current_position.y+modifier.y):
                #new direction is not going backwards and can move to location
                current_position.x += modifier.x
                current_position.y += modifier.y
                #current_direction
                path.append([Position(current_position.x,current_position.y),current_direction])
                ...
                if self.check_move(current_position.x+self.moves['u'].x,current_position.y+self.moves['u'].y) and (old_direction != 'd' and current_direction != 'u'): #(current_direction == 'l' or current_direction == 'r')
                    modifier = Position(self.moves['u'].x, self.moves['u'].y)
                    current_position.x += modifier.x
                    current_position.y += modifier.y
                    print('   ELIF UP - Current Direction - ' + current_direction)
                    current_direction = 'u'
                    path.append([Position(current_position.x,current_position.y),current_direction])
                    print('     Other Direction: UP X ' + self.coord_to_index(current_position.x) + ' Y ' + self.coord_to_index(current_position.y))
                elif self.check_move(current_position.x+self.moves['d'].x,current_position.y+self.moves['d'].y) and (old_direction == 'u' or current_direction == 'd'): #current_direction == 'l' or current_direction == 'r'
                    modifier = Position(self.moves['d'].x, self.moves['d'].y)
                    current_position.x += modifier.x
                    current_position.y += modifier.y
                    print('   ELIF UP - Current Direction - ' + current_direction)
                    current_direction = 'd'
                    path.append([Position(current_position.x,current_position.y),current_direction])
                    print('     Other Direction: DOWN X ' + self.coord_to_index(current_position.x) + ' Y ' + self.coord_to_index(current_position.y))
                elif self.check_move(current_position.x+self.moves['l'].x,current_position.y+self.moves['l'].y) and (old_direction == 'r' or current_direction == 'l'): #current_direction == 'd' or current_direction == 'u'
                    modifier = Position(self.moves['l'].x, self.moves['l'].y)
                    current_position.x += modifier.x
                    current_position.y += modifier.y
                    print('   ELIF UP - Current Direction - ' + current_direction)
                    current_direction = 'l'
                    path.append([Position(current_position.x,current_position.y),current_direction])
                    print('     Other Direction: LEFT X ' + self.coord_to_index(current_position.x) + ' Y ' + self.coord_to_index(current_position.y))
                elif self.check_move(current_position.x+self.moves['r'].x,current_position.y+self.moves['r'].y) and (old_direction == 'l' or current_direction == 'r'): #current_direction == 'd' or current_direction == 'u'
                    modifier = Position(self.moves['r'].x, self.moves['r'].y)
                    current_position.x += modifier.x
                    current_position.y += modifier.y
                    print('   ELIF UP - Current Direction - ' + current_direction)
                    current_direction = 'r'
                    path.append([Position(current_position.x,current_position.y),current_direction])
                    print('     Other Direction: RIGHT X ' + self.coord_to_index(current_position.x) + ' Y ' + self.coord_to_index(current_position.y))
                #new direction would mean going backwards
                #find appropriate new direction

            if not self.check_collision_screen_boundaries(Position(current_position.x,current_position.y)):
                in_bounds = False
        ...
        #check if latest coord is out of bounds
        if not in_bounds:
            return False #Path has gone out of bounds so path finding has failed
